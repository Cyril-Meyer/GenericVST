...
def  main():
  ...
  installer = "\installer\GenericVST Installer.exe"
   
  if demo:
    installer = "\installer\GenericVST Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\GenericVST manual.pdf" 
  ]
  ...
  ZIPNAME = "GenericVST-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "GenericVST-v" + FULLVERSIONSTR + "-win-demo.zip"
