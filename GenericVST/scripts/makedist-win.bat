...
if %1 == 1 (echo Making GenericVST Windows DEMO VERSION distribution ...) else (echo Making GenericVST Windows FULL VERSION distribution ...)
...
REM - msbuild GenericVST-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild GenericVST.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild GenericVST.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\GenericVST.aaxplugin\Contents\Win32\GenericVST.aaxplugin --out .\build-win\aax\bin\GenericVST.aaxplugin\Contents\Win32\GenericVST.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\GenericVST.aaxplugin\Contents\x64\GenericVST.aaxplugin --out .\build-win\aax\bin\GenericVST.aaxplugin\Contents\x64\GenericVST.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\GenericVST.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\GenericVST.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "GenericVST Installer" ".\installer\GenericVST Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\GenericVST Installer.exe" ".\installer\GenericVST Demo Installer.exe"
REM -del ".\installer\GenericVST Installer.exe"
REM -)
